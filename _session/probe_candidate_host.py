#!/usr/bin/env python3
"""Probe portalCore getLiveData on candidate host (probe_host_quick pattern)."""
...
HOST = sys.argv[1] if len(sys.argv) > 1 else "sfgknh.qho3cnsyil.com"
...
BODY = {
    ...
    "portalCode": "masnew",
    "userId": "169355704",
    "userToken": "6da3c458-b2de-4798-86a7-57028fb25b27",
    "columnId": 76182,
...
def probe(scheme: str) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30, context=ctx) as resp:
            ...
            print(f"{scheme.upper()} {resp.status} host={HOST}")
            print(dec[:500])
    except Exception as e:
        print(f"{scheme.upper()} ERR host={HOST}: {e}")


if __name__ == "__main__":
    print(f"Probing {HOST}{PATH}")
    for s in ("https", "http"):
        probe(s)
